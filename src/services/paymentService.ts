...
export const stripePromise = key
  ? loadStripe(key).catch((error) => {
      console.error("Failed to load Stripe.js:", error);
      console.warn(
        "Make sure you have internet connection and Stripe key is correct"
      );
      return null;
    })
  ...
  async getPaymentMethods(): Promise<PaymentMethod[]> {
    try {
      ...
      if (error.response?.status === 404 || error.code === "ERR_NETWORK") {
        console.warn(
          "Backend endpoint /saved-cards not available yet. Using empty list."
        );
        return [];
      }
      return [];
